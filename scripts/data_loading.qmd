...
con_dwh <- get_sqlServerCon_by_windows_auth(server_name = "SCUS-PF1K22N9",
 db_name = "DUNIN_DWH"
)
```

```{r eval=FALSE}
DBI::dbWriteTable(con_dwh,"dbo.dim_dates",dim_dates |> distinct(),append = TRUE)
DBI::dbWriteTable(con_dwh,"dbo.dim_clients",dim_clients  |> distinct(),append = TRUE)
DBI::dbWriteTable(con_dwh,"dbo.dim_representants",dim_representants  |> distinct(),append = TRUE)
DBI::dbWriteTable(con_dwh,"dbo.dim_adresses",dim_adresses  |> distinct(),append = TRUE)
DBI::dbWriteTable(con_dwh,"dbo.dim_projets",dim_projets  |> distinct(),append = TRUE)
DBI::dbWriteTable(con_dwh,"dbo.fact_etapes",fact_etapes |> distinct(),append = TRUE)
DBI::dbWriteTable(con_dwh,"dbo.fact_lumieres",fact_lumieres |> distinct(),append = TRUE)
DBI::dbWriteTable(con_dwh,"dbo.transactions",transactions |> distinct(),append = TRUE)
```
# Conclusion
